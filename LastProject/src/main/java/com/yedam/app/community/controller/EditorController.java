...
public class EditorController {
	...
	@ResponseBody
	public ModelAndView attach(MultipartHttpServletRequest request) {
		// modelandview를 사용하여 json 형식으로 보내기위해 모델앤뷰 생성자 매개변수로 jsonView 라고 써줌
		// jsonView 라고 쓴다고 무조건 json 형식으로 가는건 아니고 @Configuration 어노테이션을 단
		// WebConfig 파일에 MappingJackson2JsonView 객체를 리턴하는 jsonView 매서드를 만들어서 bean으로
		// 등록해야함
		...
		String saveName = uploadAttachPath + File.separator + uploadFileName;
		...
		// DB에 저장할 때 java에서만 읽히는 File.separator를 /로 변환 후 DB에 저장
		String imagePath = "/attachs/" + uploadFileName.replace(File.separator, "/");
